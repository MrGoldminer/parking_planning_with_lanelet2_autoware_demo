...
void buildGraph() {
    ...
    if (max_component_size < adjacency_list_.size() * 0.8) {
        ROS_WARN("[WARN] Graph is fragmented! Only %.1f%% nodes are in the main component.",
                 100.0 * max_component_size / adjacency_list_.size());
...
std::vector<long long> bfs(long long start, long long goal) {
    ...
    if (adjacency_list_.find(start) == adjacency_list_.end()) {
        ROS_ERROR("[ERR] Start node %lld not found in adjacency_list!", start);
        // 尝试找到最近的替代起点
    ...
    if (adjacency_list_.find(goal) == adjacency_list_.end()) {
        ROS_ERROR("[ERR] Goal node %lld not found in adjacency_list!", goal);
        // 尝试找到最近的替代终点
        double min_dist = 1e18;
    ...
    while (!q.empty()) {
        ...
        if (current == goal) {
            ROS_INFO("[OK] Path found! Visited %d nodes.", visited_count);
            break;
        }

        // [OK] 检查adjacency_list_[current]是否存在
    ...
    if (came_from.find(goal) == came_from.end()) {
        ROS_ERROR("[ERR] No path found from %lld to %lld (visited %d nodes)",
    ...
    ROS_INFO("[OK] Path found with %lu waypoints", path.size());
    return path;
}
...
std::vector<long long> findRelationPath(long long start_rel, long long goal_rel) {
    ...
    // [OK] 检查relation adjacency
    if (relation_adjacency_.find(start_rel) == relation_adjacency_.end()) {
        ROS_ERROR("[ERR] Start relation %lld not found in relation_adjacency_!", start_rel);
        return std::vector<long long>();
    }

    if (relation_adjacency_.find(goal_rel) == relation_adjacency_.end()) {
        ROS_ERROR("[ERR] Goal relation %lld not found in relation_adjacency_!", goal_rel);
        return std::vector<long long>();
    }
    ...
    while (!q.empty()) {
        ...
        if (cur == goal_rel) {
            ROS_INFO("[OK] Relation path found! Visited %d relations.", visited);
            break;
        }
    ...
    if (came_from.find(goal_rel) == came_from.end()) {
        ROS_ERROR("[ERR] No relation path found (visited %d relations)", visited);
        return path;
    }
    ...
    ROS_INFO("[OK] Relation path: %lu lanelets", path.size());
    return path;
}
...
void planTask() {
    ...
    if (start_rel) {
        ...
        if (!candidate_starts.empty()) {
            ...
            ROS_INFO("[OK] Selected start node %lld with %lu neighbors (dist=%.2fm)",
                     start_node_id,
    ...
    if (park_rel) {
        ...
        if (!candidate_goals.empty()) {
            ...
            ROS_INFO("[OK] Selected goal node %lld with %lu neighbors (dist=%.2fm)",
                     target_node,
...
if (global_plan_.empty()) {
    ROS_ERROR("[ERR] All planning methods failed! Generating emergency straight-line path.");
    ...
    ROS_WARN("[WARN] Emergency path generated with %lu points", global_plan_.size());
}

...
bool OSMMapLoader::loadFromFile(const std::string& filename) {
    ...
    ROS_INFO("[OK] Loaded %lu nodes, %lu ways, %lu relations",
...
void OSMMapLoader::projectToLocal() {
    ...
    ROS_INFO("[OK] Projected to local coordinates (origin: lat=%.6f, lon=%.6f)",
...
void OSMMapLoader::extractParkingSpots() {
    ...
    ROS_INFO("[OK] Extracted %lu parking spots", parking_spots_.size());
}

...
std::vector<long long> PathPlanner::planRelationPath(long long start_rel, long long goal_rel) {
    ...
    // 调试：打印起点和终点的邻居
    ROS_INFO("Start relation %lld has %lu neighbors:", start_rel, rel_adjacency.at(start_rel).size());
    for (long long neighbor : rel_adjacency.at(start_rel)) {
        ROS_INFO("  - %lld", neighbor);
    }

    ROS_INFO("Goal relation %lld has %lu neighbors:", goal_rel, rel_adjacency.at(goal_rel).size());
    for (long long neighbor : rel_adjacency.at(goal_rel)) {
        ROS_INFO("  - %lld", neighbor);
    }
